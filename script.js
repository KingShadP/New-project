...
function setupHeader() {
  // ⚡ Bolt: Cache state to prevent redundant DOM updates and toggle thrashing
  let isScrolled = null;
  // ⚡ Bolt: Track animation frame for throttling scroll events
  let frame = null;
  ...
  window.addEventListener(
    "scroll",
    () => {
      // ⚡ Bolt: Throttle scroll events with requestAnimationFrame to fix layout thrashing
...
function setupNavState() {
  ...
  // ⚡ Bolt: Cache active section to avoid recalculating nav indicator positions repeatedly
  ...
  const updateNav = () => {
    ...
    // ⚡ Bolt: Only update DOM and calculate layout if active section actually changed

...
function setupParallax() {
  ...
  const cacheParallaxData = () => {
    viewportHeight = window.innerHeight;
    intensity = Number(motionConfig.intensity ?? 1);

    // Performance optimization: Cache target dimensions to avoid layout thrashing during scroll
  ...
  const updateParallax = () => {
    ...
    parallaxData.forEach((data) => {
      ...
      const progressValue =
        (currentTop + data.rectHeight / 2 - viewportHeight / 2) /
        viewportHeight;
      const offset = Math.max(
        -Math.abs(data.depth),
        Math.min(Math.abs(data.depth), progressValue * -data.depth),
      );
